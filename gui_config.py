...
def encrypt_value(value):
    """Devuelve un valor encriptado."""
    return fernet.encrypt(value.encode()).decode()


def decrypt_value(value):
    """Devuelve un valor desencriptado, si está encriptado."""
    try:
        return fernet.decrypt(value.encode()).decode()
    except:
        return value  # Si no está cifrado, se deja igual
...
def abrir_gui_pro():
    ...
    def click_guardar():
        ...
        # Validación básica: PING_INTERVAL
        try:
            int(ping.get())
        except ValueError:
        ...
        # Si no hubo errores, guardar config
        values = {
...
from ad_utils import validar_ad
from logs_utils import eliminar_logs
from db_conexion import conectar_sql   # <-- Validación SQL agregada
